                    ...
                    <div className="bg-white p-2 rounded border-2 border-blue-300">
                        ...
                        {selectedEntity.type === 'ellipse' && (
                            <div className="mt-3 border-t pt-2">
                                ...
                                <div className="prop-row"><label className="prop-label">回転°</label><NumberInput value={selectedEntity.rotation !== undefined ? selectedEntity.rotation : 0} onChange={e => updateEntity('rotation', ((Number(e.target.value) % 360) + 360) % 360)} className="prop-input" /></div>
                                <div className="prop-row"><label className="prop-label">開始角°</label><NumberInput value={selectedEntity.startAngle !== undefined ? selectedEntity.startAngle : 0} onChange={e => updateEntity('startAngle', ((Number(e.target.value) % 360) + 360) % 360)} className="prop-input" /></div>
                                <div className="prop-row"><label className="prop-label">終了角°</label><NumberInput value={selectedEntity.endAngle !== undefined ? selectedEntity.endAngle : 360} onChange={e => { const n = ((Number(e.target.value) % 360) + 360) % 360; updateEntity('endAngle', n === 0 ? 360 : n); }} className="prop-input" /></div>
